...
std::unique_ptr<CharacterCompositor> CharacterCompositor::create(
    ...
    auto manifest = CharacterManifest::load(
        character_root, preferred_scale, error_message
    );
        ...
        if (group->kind == DrawGroupKind::Hair) {
            ...
            for (const std::size_t layer_index : group->layer_indices) {
                ...
                if (layer.id == expression.eyes_layer_id) {
                    ...
                    include_asset_bounds(layer, expression.eyes_user_asset_id);
                    ...
                    include_asset_bounds(layer, expression.mouth_smile_asset_id);
                }
            }
...
void CharacterCompositor::start_enter() {
    ...
    if (manifest_.expression.enabled) {
        if (was_hidden) {
            expression_animator_.reset_inward();
        } else {
...
void CharacterCompositor::ensure_tick() {
    if (tick_callback_id_ != 0 || tick_widget_ == nullptr || !animator_.active()) {
        return;
    }
...
gboolean CharacterCompositor::on_tick(GdkFrameClock* frame_clock) {
    if (frame_clock == nullptr || !animator_.active()) {
        tick_callback_id_ = 0;
        last_frame_time_us_ = 0;
    ...
    const double delta_seconds = static_cast<double>(
        frame_time_us - last_frame_time_us_
    ) / 1'000'000.0;
    last_frame_time_us_ = frame_time_us;

    if (animator_.advance(delta_seconds)) {
        apply_motion_state();
        queue_lifecycle_draw(frame_time_us);
    }
    if (animator_.active()) return G_SOURCE_CONTINUE;
...
const std::string& CharacterCompositor::selected_asset_id(
    ...
    if (layer.id == rig.eyes_layer_id) {
        ...
        case CharacterEyeFrame::User:
            return rig.eyes_user_asset_id;
    ...
    if (layer.id == rig.mouth_layer_id) {
        return expression.mouth == CharacterMouthFrame::Smile
            ? rig.mouth_smile_asset_id
...
bool CharacterCompositor::load_surfaces(std::string* error_message) {
    ...
    if (manifest_.expression.enabled) {
        ...
        required_assets.insert(manifest_.expression.eyes_user_asset_id);
        ...
        required_assets.insert(manifest_.expression.mouth_smile_asset_id);
    }
...
bool CharacterCompositor::build_hair_meshes(std::string* error_message) {
    ...
    for (const auto& layer : manifest_.layers) {
        ...
        if (!build_hair_node_cache(cache, layer.id, error_message)) {
            return false;
        }
...
void CharacterCompositor::append_hair_mesh_geometry(
    ...
    if (snapshot == nullptr || cache.texture == nullptr) return;
    ...
    for (const auto& band : cache.bands) {
...
bool CharacterCompositor::build_hair_node_cache(
    ...
    cache.quantized_nodes.reserve(static_cast<std::size_t>(node_count));
    for (int index = 0; index < node_count; ++index) {
        ...
        append_hair_mesh_geometry(snapshot, cache, 0.0, 0.0, offset);
        ...
        cache.quantized_nodes.push_back(std::move(node));
    }
    return true;
}
