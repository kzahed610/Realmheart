...
Pixel sample_bilinear(const Argb32ImageView& source, double x, double y) {
    if (x < -1.0 || y < -1.0 || x > source.width || y > source.height) return {};
...
std::pair<double, double> flow_vector(const Argb32ImageView& flow, int x, int y) {
...
std::optional<std::vector<std::uint8_t>> warp_argb32(
    ...
    if (!valid_view(source) || !valid_view(flow) || !valid_view(movement_mask)) {
        return fail("Flow warp received an invalid ARGB32 image view");
    }
    if (source.width != flow.width || source.height != flow.height ||
        source.width != movement_mask.width || source.height != movement_mask.height) {
        return fail("Source, flow map, and movement mask dimensions must match");
    }
    if (!std::isfinite(displacement_pixels) || std::abs(displacement_pixels) > 16.0) {
