...
// Built-in emoji index used as a fallback when the shipped
// `config/hypr/hyprland/scripts/fuzzel-emoji.sh` is absent, unreadable, or does
// not contain the `### DATA ###` marker. The launcher parses this with the same
// code path as the on-disk file (glyph + whitespace-separated keywords per
// line), so users who drop a custom fuzzel-emoji.sh into their config still
// override the set below.
//
// Carries the full ~1900 glyph index so the picker never degrades even on a
// completely fresh account with zero installed data.
inline constexpr std::string_view kEmojiDataFallback = R"DATA(### DATA ###
...
🪎 treasure chest gold loot pirate)DATA";
