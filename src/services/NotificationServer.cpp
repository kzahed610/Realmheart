...
std::uint32_t NotificationServer::notify(
    ...
    std::uint32_t retired_id = 0;
    {
        ...
        id = replaces_id != 0 ? replaces_id : allocate_id();
        if (const auto existing = active_ids_.find(id); existing != active_ids_.end()) {
            ...
            retired_id = active_order_.front();
            active_order_.pop_front();
            active_ids_.erase(retired_id);
        }
    ...
    if (retired_id != 0) {
        if (closed_handler_) closed_handler_(retired_id, 4);
        if (closed_observer_) closed_observer_(retired_id, 4);
    }
...
std::uint32_t NotificationServer::allocate_id() {
    for (;;) {
        ...
        if (candidate != 0 && !active_ids_.contains(candidate)) return candidate;
    }
}
