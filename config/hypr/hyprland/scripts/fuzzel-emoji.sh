#!/usr/bin/env bash
# fuzzel-emoji.sh — Realmheart emoji database.
#
# The Realmheart launcher reads this file directly to power its emoji picker.
# Lines after the marker `### DATA ###` are parsed as `<glyph> <keywords>`
# (whitespace-separated); the glyph is copied to the clipboard, the keywords
# are matched against the launcher query.
#
# To extend: add new emoji lines below the marker. Keep glyph first, then
# space-separated keywords (ASCII). Anything above the marker is treated as
# script documentation and ignored.
