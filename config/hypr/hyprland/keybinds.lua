...
hl.bind("SUPER + Tab", hl.dsp.exec_cmd(realmheartCommand .. "workspace-overview-toggle"), { description = "Shell: Workspace Overview" })
hl.bind("SUPER + L", hl.dsp.exec_cmd(realmheartCommand .. "lock-session"), { description = "Lock session" })
hl.bind(
    "SUPER + V",
    hl.dsp.exec_cmd(
        realmheartCommand .. "launch-launcher-query '>clip '"
    ),
    { description = "Utilities: Clipboard history >> Realmheart" }
)
hl.bind(
    "SUPER + Period",
    hl.dsp.exec_cmd(
        realmheartCommand .. "launch-launcher-query '>emoji '"
    ),
    { description = "Utilities: Emoji >> Realmheart" }
)
hl.bind("SUPER + N", hl.dsp.exec_cmd(realmheartCommand .. "sidebar-right-toggle"), { description = "Shell: Toggle right sidebar" })
hl.bind("SUPER + A", hl.dsp.exec_cmd(realmheartCommand .. "sidebar-right-toggle"))
hl.bind("SUPER + G", hl.dsp.exec_cmd(realmheartCommand .. "toggle-notes"), { description = "Shell: Toggle notes" })
hl.bind("SUPER + J", hl.dsp.exec_cmd(realmheartCommand .. "bar-toggle"), { description = "Shell: Toggle bar" })
hl.bind("CTRL + ALT + Delete", hl.dsp.exec_cmd(realmheartCommand .. "logout-menu || wlogout -p layer-shell"),
    { description = "Shell: Toggle session menu" })

hl.bind("XF86MonBrightnessUp", hl.dsp.exec_cmd("brightnessctl s 5%+ && " .. realmheartCommand .. "osd-brightness"),
    { locked = true, repeating = true })
hl.bind("XF86MonBrightnessDown", hl.dsp.exec_cmd("brightnessctl s 5%- && " .. realmheartCommand .. "osd-brightness"),
    { locked = true, repeating = true })
hl.bind("XF86AudioRaiseVolume", hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 2%+ -l 1.5 && " .. realmheartCommand .. "osd-volume"),
    { locked = true, repeating = true })
hl.bind("XF86AudioLowerVolume", hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 2%- && " .. realmheartCommand .. "osd-volume"),
    { locked = true, repeating = true })

--# Direct picker fallback: realmheartCommand .. "set-wallpaper"
hl.bind("CTRL + SUPER + T", hl.dsp.exec_cmd(realmheartCommand .. "mana-cores-toggle"), { description = "Shell: Change wallpaper" })
...
hl.bind(
    ...
    {
        description = "Toggle Realmheart sidebar character"
    }
)

--##! Realmheart lockscreen jail
-- Registered EMPTY: no binds inside. Realmheart enters this submap while
-- the Broken Seal lockscreen is up, so every compositor keybind (SUPER+num
-- workspace peeks included) goes dead while locked. The lockscreen's
-- layer-shell keyboard grab keeps the password entry working. Realmheart
-- dispatches `submap reset` on unlock.
hl.define_submap("realmheart-locked", function()
    -- The bridge only registers a submap that declares at least one bind,
    -- so the jail contains a single no-op self-bind: the combo merely
    -- re-enters the same jail. It exists so the submap exists; it unlocks
    -- nothing and toggles nothing.
    hl.bind("SUPER + ALT + F7", hl.dsp.submap("realmheart-locked"))
end)
