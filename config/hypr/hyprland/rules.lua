...
-- Realmheart
hl.layer_rule({ match = { namespace = "realmheart-.*" }, blur = true})
hl.layer_rule({ match = { namespace = "realmheart-.*" }, ignore_alpha = 0.79})
-- Broken Seal lockscreen: NO blur — the lock stage is an empty workspace
-- (wallpaper only, zero windows), so there is nothing to hide. Explicit
-- override beats the generic realmheart-* rule above. Instant map/unmap:
-- the seal's own GL art handles all motion (mana-core style, one animator).
hl.layer_rule({ match = { namespace = "realmheart-broken_seal" }, blur = false, ignore_alpha = 0, no_anim = true})
hl.layer_rule({ match = { namespace = "realmheart-bar" }, animation = "slide"})
hl.layer_rule({ match = { namespace = "realmheart-right-sidebar" }, animation = "slide right"})
hl.layer_rule({ match = { namespace = "realmheart-notes" }, animation = "popin 90%"})
